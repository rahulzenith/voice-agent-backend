...
async def cancel_appointment(
    ...
    """
    logger.info(f"🔧 TOOL CALLED: cancel_appointment with appointment_id={appointment_id}")
    ...
    try:
        ...
        result = client.table("appointments").delete().eq("id", appointment_id).execute()

        if result.data:
            # Mark the slot as available again
            if slot_id:
                client.table("slots").update({"is_available": True}).eq("id", slot_id).execute()
        ...
        return "I'm sorry, I had trouble cancelling the appointment. Could you try again?"
