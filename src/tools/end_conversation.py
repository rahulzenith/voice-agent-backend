...
async def end_conversation(context: RunContext) -> str:
    ...
    try:
        ...
        try:
            ...
            if session and hasattr(session, '_chat_ctx') and hasattr(session._chat_ctx, 'items'):
                chat_items = session._chat_ctx.items
                transcript_messages = TranscriptService.extract_from_chat_context(chat_items)
                transcript_text = TranscriptService.format_for_display(transcript_messages)
            else:
                # Fallback: Use shared state
                conversation_messages = getattr(shared_state, 'conversation_messages', [])
        ...
        session_duration = 0
        if hasattr(shared_state, "session_start_time") and shared_state.session_start_time:
        ...
        if hasattr(shared_state, "usage_collector") and shared_state.usage_collector:
            try:
                ...
                cost_breakdown = CostService.calculate_from_usage_summary(usage_summary, session_duration)
            except Exception as e:
        ...
        assistant_messages = [m for m in transcript_messages if m.get("role") == "assistant"]
        ...
        await EventService.emit_tool_call(room, "end_conversation", "success", {"summary": summary_text})
        ...
        await EventService.emit_tool_call(room, "end_conversation", "error", {"error": str(e)})
