...
@function_tool
async def fetch_slots(context: RunContext, specific_date: str = "") -> str:
    """
    ...
    try:
        ...
        if specific_date:
            ...
            slots_query = (
                client.table("slots")
                .select("*")
                .gte("slot_date", today_ist.isoformat())
        ...
        if not slots_result.data:
            await EventService.emit_tool_call(
                room, "fetch_slots", "success", {"available_slots": []}
            )
            if specific_date:
        ...
        for slot in slots_result.data:
            ...
            if (
                not specific_date
                and slot_date == today_ist
                and slot_time <= current_time_ist
            ):
                continue
        ...
        if specific_date:
            ...
            if slots_by_time["afternoon"]:
                response_parts.append(
                    f"Afternoon: {', '.join(slots_by_time['afternoon'])}"
                )
            ...
            response = (
                f"I have 3 nearest slots available at: {', '.join(slot_descriptions)}."
            )
        ...
        await EventService.emit_tool_call(
            room, "fetch_slots", "error", {"error": str(e)}
        )
