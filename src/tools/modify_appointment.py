...
async def modify_appointment(
    ...
    """
    logger.info(
        f"🔧 TOOL CALLED: modify_appointment with appointment_id={appointment_id}, new_date={new_date}, new_time={new_time}"
    )
    ...
    try:
        if not contact_number:
            await EventService.emit_tool_call(
                room, "modify_appointment", "error", {"error": "User not identified"}
            )
        ...
        if not appointment.data:
            await EventService.emit_tool_call(
                room, "modify_appointment", "error", {"error": "Appointment not found"}
            )
        ...
        if not new_slot_query.data:
            await EventService.emit_tool_call(
                room, "modify_appointment", "error", {"error": "New slot not found"}
            )
        ...
        if result.data:
            ...
            shared_state.user_preferences = PreferenceTracker.update_preferences(
                shared_state.user_preferences, new_date, new_time
            )
            ...
            await EventService.emit_tool_call(
                room, "modify_appointment", "success", {"appointment": result.data[0]}
            )
        ...
        return (
            "I'm sorry, I had trouble modifying the appointment. Could you try again?"
        )

    except Exception as e:
        logger.error(f"Error modifying appointment: {e}")
        await EventService.emit_tool_call(
            room, "modify_appointment", "error", {"error": str(e)}
        )
        return (
            "I'm sorry, I had trouble modifying the appointment. Could you try again?"
        )
