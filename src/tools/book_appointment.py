...
async def book_appointment(
    ...
    """
    logger.info(f"🔧 TOOL CALLED: book_appointment with date={appointment_date}, time={appointment_time}")
    ...
    try:
        ...
        slot_query = client.table("slots").select("*").eq(
            "slot_date", appointment_date
        ).eq("slot_time", appointment_time).execute()
        ...
        user_existing = client.table("appointments").select("*").eq(
            "slot_id", slot_id
        ...
        try:
            ...
            if result.data:
                ...
                try:
                    client.table("slots").update({"is_available": False}).eq("id", slot_id).execute()
                    logger.info(f"✅ Slot {slot_id} marked as unavailable")
                except Exception as update_error:
                    # Slot update failed, but appointment is booked - log and continue
                    logger.warning(f"Failed to update slot availability: {update_error}")
            ...
            is_unique_violation = (
                error_code == '23505' or  # PostgreSQL unique violation code
                '23505' in error_str or  # Sometimes code is in string
                'unique constraint' in error_str or
                'duplicate key' in error_str or
                'duplicate key value' in error_str or
                ('unique' in error_str and ('slot_id' in error_str or 'appointment_date' in error_str or 'appointment_time' in error_str)) or
            ...
            if is_unique_violation:
                ...
                date_time_existing = client.table("appointments").select("*").eq(
                    "appointment_date", appointment_date
                ).eq("appointment_time", appointment_time).execute()
                ...
                logger.warning(f"Race condition detected: Slot {slot_id} was booked by another user")
                if room:
                    await EventService.emit_tool_call(
                        room,
                        "book_appointment",
                ...
                logger.error(f"Error inserting appointment: {insert_error}", exc_info=True)
                raise
